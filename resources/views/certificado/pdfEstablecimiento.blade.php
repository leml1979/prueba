...
<body>
  <header><img src="{{ public_path('img/banner.png') }}" width="90%" height="50%"/></header>
  <footer></footer>
  <main>
    <p class="text-right">Caracas, {!! $fecha->day!!} del mes de {!! $fecha->locale("es")->monthName !!} de {!! $fecha->year !!}</p>
    ...
    <p class="text-justify">Quien suscribe, ENEIDA LAYA LUGO, titular de la cédula de identidad N° V-11.366.874, actuando en mi carácter de SUPERINTENDENTE NACIONAL PARA LA DEFENSA DE LOS DERECHOS SOCIO ECONÓMICOS, designación efectuada mediante Decreto N° 4.016 de fecha 29 de Octubre de 2019, publicado en la Gaceta Oficial de la República Bolivariana de Venezuela Nº 41.748, de fecha 29 de Octubre de 2019; por medio del presente instrumento, actuando de conformidad con la atribución prevista en el artículo 11, numeral 10 del Decreto con Rango, Valor y Fuerza de Ley Orgánica de Precios Justos, certifica la inscripción ante el Registro Único de Personas que Desarrollan Actividades Económicas del Sujeto de Aplicación <b> {!! mb_strtoupper($establecimiento->sujetos->sujeto,"UTF-8") !!}</b> con el RIF <b>{!! $establecimiento->sujetos->rif !!}</b> en fecha {!! $establecimiento->sujetos->registro_dia_sundde."-".$establecimiento->sujetos->registro_mes_sundde."-".$establecimiento->sujetos->registro_anio_sundde !!} con el certicado Nº <b>{!! $establecimiento->sujetos->codigo_certificado !!}</b>, que desarrolla las siguiente acividad economica principal <b>{!! $establecimiento->sujetos->clase->clase !!}</b> del siguiente establecimiento denominado <b>{!! $establecimiento->establecimiento !!}</b> en fecha <b>{!! \Carbon\Carbon::createFromTimestamp($establecimiento->fecha_creacion)->format("d-m-Y")  !!}</b> con el certicado Nº <b>{!! $establecimiento->codigo_certificado !!}</b>, que desarrolla las siguiente actividad economica principal <b>{!! $establecimiento->sede->sede !!}</b>.</p>
